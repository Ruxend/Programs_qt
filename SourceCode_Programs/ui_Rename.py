...
class ui_Rename():
	...
	def setupUi(self, QMainWindow):
		self.setWindowTitle("Tool of ExcelToFilename") 			# 设置窗口标题
		...
		for self.index, self.r in enumerate(conText):
			for self.cindex, self.c in enumerate(self.r):
				if self.c == "请选择Excel对象文件:":
					...
					self.browse_1.clicked.connect(self.get_entry_1)  		# 连接槽
					...
					self.browse_2.clicked.connect(self.get_entry_3)  		# 连接槽
					self.groupbox_1_grid_1.addWidget(self.browse_2, self.index, self.cindex + 7)
				elif self.c == "请输入对象文件格式:":
					self.label_4 = QLabel()
					...
					self.entry_4.setPlaceholderText("Please input here(It's up to you)") 	# 设置浮显文字
