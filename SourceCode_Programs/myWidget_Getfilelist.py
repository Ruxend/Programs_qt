...
from threads_Getfilelist import WorkThread_Getfilelist

class myWidget_Getfilelist(QMainWindow, ui_Getfilelist):
	signal_child_1 = pyqtSignal()
	def __init__(self, parent=None):
		super().__init__(parent)
		self.setupUi(self)
		self.btn_confirm.clicked.connect(self.Getfilelist_confirm)  	# 连接槽
		QShortcut(QKeySequence("Return"), self, self.Getfilelist_confirm)
	...
	def get_entry_4(self):
		dire = QFileDialog.getExistingDirectory(self, "选取文件夹", os.getcwd())
		self.entry_4.setText(dire)

	def Getfilelist_confirm(self):
		...
		self.entry_5_value = self.entry_5.text()
		if self.entry_1_value=="" or self.entry_2_value=="" or self.entry_3_value=="" or self.entry_4_value=="": 
	...
	# def Getfilelist_quit(self):
	# 	self.quit = QMessageBox.question(self, "Quit", "Do you want to quit ?", QMessageBox.Ok | QMessageBox.Cancel, QMessageBox.Ok)
	# 	if self.quit == QMessageBox.Ok:
	# 		self.close()
	...
	def work_start(self):
		...
		self.thread = WorkThread_Getfilelist(entry_1=self.entry_1_value, entry_2=self.entry_2_value, 
								entry_3=self.entry_3_value, entry_4=self.entry_4_value, 
								entry_5=self.entry_5_value, text=self.text)
	...
	def signal_1_call(self, ofp, ostr):
		self.text.update()
		self.text.append("From <{}{}> get <{}> successfully!".format(ofp, ostr, ostr.split(".")[0]))  # 打印输出内容
		self.refresh_color()
	...
	def keyPressEvent(self, e):
		...
		if e.key() == Qt._Key_Return:
			self.Getfilelist_confirm()
		elif e.key() == Qt.Key_Escape:
			self.close()
	
# if __name__ == "__main__":
# 	app = QApplication(sys.argv)
# 	window = myWidget_Getfilelist()
# 	window.show()
# 	sys.exit(app.exec())
